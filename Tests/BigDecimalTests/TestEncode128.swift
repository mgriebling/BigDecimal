...
final class TestEncode128: XCTestCase {
    ...
    static func U128(_ x: String) -> UInt128 {
        ...
        return UInt128(UInt64(x[x.startIndex ..< mid], radix: 16)!, UInt64(x[mid ..< x.endIndex], radix: 16)!)
    }
    ...
    func test1() {
        for t in tests1 {
            ...
            XCTAssertEqual(Decimal128(BigDecimal(t.x)).asUInt128(.dpd).hi, t.dec.hi)
            XCTAssertEqual(Decimal128(BigDecimal(t.x)).asUInt128(.dpd).lo, t.dec.lo)
        }
