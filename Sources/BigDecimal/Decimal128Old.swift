...
struct Decimal128Old {
    ...
    init(_ value: UInt128, _ encoding: BigDecimal.Encoding = .dpd) {
        ...
        if x == 0x1f {
            ...
            if sig > Decimal128Old.maxSignificand {
                sig = UInt128.zero
            }
            self.exponent = exp
    ...
    func asUInt128(_ encoding: BigDecimal.Encoding = .dpd) -> UInt128 {
        ...
        if self.isNan {
            return Decimal128Old.nan
        } else if self.isInfinite {
            return self.sign ? Decimal128Old.minusInf : Decimal128Old.plusInf
        }
        assert(self.exponent <= 0x2fff)
        assert(self.significand <= Decimal128Old.maxSignificand)
